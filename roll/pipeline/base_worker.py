...
class ActorWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def apply_lora_state_dict_warm_start(self, lora_state_dict: Dict, num_training_steps: int,
                                         missing_param_policy: str = "kaiming_zero"):
        """SVD warm-start: load externally-computed LoRA tensors into the model."""
        with Timer("apply_lora_state_dict_warm_start") as total_timer:
            exec_metrics: Dict = self.strategy.apply_lora_state_dict_warm_start(
                lora_state_dict, num_training_steps, missing_param_policy=missing_param_policy,
            )
        metrics = {
